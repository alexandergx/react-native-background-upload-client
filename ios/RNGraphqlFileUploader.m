...
-(id) init {
    return [RNGraphqlFileUploader sharedInstance];
}
...
- (NSArray<NSString *> *)supportedEvents {
    return @[
        @"RNGraphqlFileUploader-progress",
        @"RNGraphqlFileUploader-error",
        @"RNGraphqlFileUploader-cancelled",
        @"RNGraphqlFileUploader-completed",
        @"RNGraphqlFileUploader-bgExpired"
    ];
}
...
RCT_REMAP_METHOD(beginBackgroundTask, beginBackgroundTaskResolver:(RCTPromiseResolveBlock)resolve
    ...
    taskId = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
        ...
        if (self.isObserving && self.bridge != nil) {
            [self sendEventWithName:@"RNGraphqlFileUploader-bgExpired" body:@{@"id": [NSNumber numberWithUnsignedLong:taskId]}];
        }
...
didCompleteWithError:(NSError *)error {
    ...
    if (error == nil) {
        ...
        if (error.code == NSURLErrorCancelled) {
            [self _sendEventWithName:@"RNGraphqlFileUploader-cancelled" body:data];
            //NSLog(@"RNBU did cancel upload %@", task.taskDescription);
        } else {
            [self _sendEventWithName:@"RNGraphqlFileUploader-error" body:data];
...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend {
    ...
    [self _sendEventWithName:@"RNGraphqlFileUploader-progress" body:@{ @"id": task.taskDescription, @"progress": [NSNumber numberWithFloat:progress] }];
}
